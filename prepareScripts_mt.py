...
name = sys.argv[1]
filesToProcess=[]
fileList = os.popen("ls " + sys.argv[2] + "/*.root").readlines()
for num in range(0,NumberOfJobs):
	miniAr=[]
	for x in fileList:
        	lastDigit=int(x.split("_")[-1].split(".")[0])
		if lastDigit%NumberOfJobs == num : miniAr.append(x)
        filesToProcess.append(miniAr)
